...
test_images = np.load('x_val.npy')
test_labels = np.load('y_val.npy')
#images = change_image_shape(images)

######################## Preprocessing ##########################
# Set channel
channel = train_images.shape[-1]
...
'''
train_tmp = np.ndarray(shape=(train_images.shape[0], 64, 64, channel))
test_tmp = np.ndarray(shape=(test_images.shape[0], 64, 64, channel))
for i in range(train_images.shape[0]):
    train_tmp[i] = cv2.resize(train_images[i], (64, 64)).reshape((64, 64, channel))
for i in range(test_images.shape[0]):
    test_tmp[i] = cv2.resize(test_images[i], (64, 64)).reshape((64, 64, channel))

x_train, y_train = train_tmp, train_labels
x_test, y_test = test_tmp, test_labels
...
ae_optimizer = Adam(lr=0.0002, beta_1=0.5, beta_2=0.9)
latent_dim=128
...
for learning_step in range(LEARNING_STEPS):
    ...
    if (learning_step + 1) % 5 == 0:
      bagan_gp.generator.save('bagan_gp_results/brain_generator_' + str(learning_step + 1) + '_.h5')
